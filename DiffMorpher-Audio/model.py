...
class DiffMorpherPipeline(StableDiffusionPipeline):
    ...
    def latent2image(self, latents, return_type='np'):
        print(f'0: {latents.shape}')
        latents = 1 / 0.18215 * latents.detach()
        print(f'1: {latents.shape}')
        ...
        if return_type == 'np':
            ...
            print(f'5: {image.shape}')
        elif return_type == "pt":
            image = (image / 2 + 0.5).clamp(0, 1)
            print(f'5-2: {image.shape}')
        print('=======================================')
        return image
        ...
        def morph(alpha_list, progress, desc):
            ...
            if attn_beta is not None:
                ...
                if save_intermediates:
                    first_image.save(f"{self.output_path}/{0:02d}.tiff", format='TIFF')
                ...
                if save_intermediates:
                    last_image.save(
                        f"{self.output_path}/{num_frames - 1:02d}.tiff", format='TIFF')
                ...
                for i in progress.tqdm(range(1, num_frames - 1), desc=desc):
                    ...
                    if save_intermediates:
                        image.save(f"{self.output_path}/{i:02d}.tiff", format='TIFF')
                    images.append(image)
                ...
                for k, alpha in enumerate(alpha_list):
                    ...
                    image = Image.fromarray(image)
                    if save_intermediates:
                        image.save(f"{self.output_path}/{k:02d}.tiff", format='TIFF')
                    images.append(image)
